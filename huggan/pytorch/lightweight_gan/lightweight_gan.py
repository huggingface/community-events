...
class Trainer():
    ...
    def set_data_src(self, folder):
        ...
        # num_workers = default(self.num_workers, math.ceil(NUM_CORES / self.accelerator.num_processes))
    ...
    def show_progress(self, num_images=4, types=['default', 'ema']):
        ...
        for checkpoint in tqdm(checkpoints, desc='Generating progress images'):
            ...
            if checkpoint == 0:
                latents = torch.randn(num_images, self.GAN.latent_dim, device=self.accelerator.device)
    ...
    def load(self, num=-1, print_version=True):
        ...
        if print_version and 'version' in load_data and self.accelerator.is_main_process:
